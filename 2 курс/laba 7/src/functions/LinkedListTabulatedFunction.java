...
public class LinkedListTabulatedFunction implements TabulatedFunction, Externalizable{
    ...
    //     s.append(this.getPoint(i).toString());
    ...
    // return s.toString();
		...
		// String s = "{";
		// for (int i = 0; i < this.getPointsCount(); i++){
		// 	s += this.getPoint(i).toString() + ", ";
		// }
		// return s += this.getPoint(this.getPointsCount() - 1) + "}";
	...
	public boolean equals(Object o) {
		...
		if (o instanceof LinkedListTabulatedFunction) {
			...
			while(last != head.next){
				last = last.next;
				if (!last.data.equals(arr.last.data)) return false;
			}
		} else {
			...
			while(last != head.next){
				last = last.next;
				if (!last.data.equals(tab.getPoint(lastIndex))) return false;
			}
		}
		return true;
	}
	...
	public Iterator<FunctionPoint> iterator() throws NoSuchElementException, UnsupportedOperationException{
		return new Iterator<FunctionPoint>() {
	        ...
	        private FunctionNode last = head.next;
			...
			public FunctionPoint next(){
				if (hasNext()) {
					FunctionNode point = last;
					last = last.next;
					return (FunctionPoint)point.data;
				}
